...
export async function loadContentWithFrontmatter(filename: string): Promise<{ content: string; frontmatter: FrontmatterData }> {
  try {
    const response = await fetch(`/content/${filename}`);
    if (!response.ok) {
      throw new Error(`Failed to load ${filename}`);
...
export async function discoverContentFiles(): Promise<ContentFile[]> {
  ...
  for (const file of defaultFiles) {
    try {
      ...
      if (!checkResponse.ok) {
        console.warn(`Content file ${file} not found (${checkResponse.status}), skipping`);
        continue;
      }
